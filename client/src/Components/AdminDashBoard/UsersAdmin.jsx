...
const UsersAdmin = () => {
  ...
  const makeAdmin = (user) => {
    ...
    fetch(RAPIDHIRE_ENDPOINT + `/users/admin/${user._id}`, {
      method: "PATCH",
    })
      .then((res) => res.json())
      .then((data) => {
  ...
  const handleDeleteUser = (id) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        fetch(RAPIDHIRE_ENDPOINT + `/delete/${id}`, {
          method: "DELETE",
        })
          .then((res) => res.json())
          .then((data) => {
  ...
  return (
    <div className="pl-4 md:pl-48">
      <div className="pt-12 pb-8">
        ...
        <p className="text-orange-700 font-bold"> Managing the Team Members </p>
      </div>
              ...
              <tr key={user._id} className="hover:bg-red-100">
                ...
                <td className="border-t px-4 py-2 ">
                  ...
                  <button
                    ...
                    className="bg-orange-500 flex gap-3 text-white px-2 py-1 rounded"
                  >
                    {" "}
                    <FaShekelSign />
