import logo2 from "../../../assets/Image/iPivotLogoOrange.png";
...
const UpperBar = () => {
  ...
  const name = user?.displayName || "Loggedin User";
  ...
  useEffect(() => {
    ...
    context.fillText(userEmail.charAt(0).toUpperCase(), canvas.width / 2, canvas.height / 2);
  ...
  //     const res = await axios.get('https://829xb1h8bj.execute-api.us-east-1.amazonaws.com/test/all-applications');
  //     setSearch(res.data);
  //   }
  //   fetchUsers();
  // },[])
  ...
  useEffect(() => {
    fetch(
      RAPIDHIRE_ENDPOINT + `/candidateSearch?search=${search}`
    )
      .then((res) => res.json())
                ...
                <nav>
                  <>
                    ...
                    <NavLink
                      to="admin/dashboard"
                      className={({ isActive }) =>
                        `flex items-center jus px-4 py-2 mt-5  transition-colors duration-300 transform  hover:bg-gray-300   hover:text-gray-700 ${isActive ? " text-orange-500" : "text-gray-600"
                        }`
                      }
                    >
      ...
      {!isAdmin && (
        <div className="flex justify-between font-bold navbar rounded-sm text-black shadow-md bg-slate-100  fixed z-10  md:flex-row  mx-auto ">
          <div className="flex  lg:md:gap-10  ">
            ...
            <div className="dropdown ">
              ...
              >
                <div className="">
                  ...
                  <li>
                    <Link to="./idCheck">
                      <FaUserPlus></FaUserPlus>ID Check
                    </Link>
                  </li>
                  <li>
            ...
            <Link to={`/`}>
              <img className="h-12 w-28  " src={logo2} alt="" />
            </Link>
          </div>
          {/* ---------------------------------------------------- end mobile view */}
          ...
          <div className="hidden lg:md:flex ">
            <ul className="menu menu-horizontal  ">
              <li>
                <Link className={isDemo ? "text-gray-400 cursor-not-allowed pointer-events-none" : ""} to={"jobs"}>JOBS</Link>
              </li>
              <li>
                <Link className={isDemo ? "text-gray-400 cursor-not-allowed pointer-events-none" : ""} to={"candidate"}>CANDIDATES</Link>
              </li>
              <li>
                <Link to={"idCheck"}>ID VERIFICATION</Link>
              </li>
              <li>
                <Link className={isDemo ? "text-gray-400 cursor-not-allowed pointer-events-none" : ""} to={"peopleSearch"}>PEOPLE SEARCH</Link>
              </li>

              <li>
                <Link className={isDemo ? "text-gray-400 cursor-not-allowed pointer-events-none" : ""} to={"company"}>COMPANY</Link>
              </li>

              {/* icons */}
              <li className={isDemo ? "text-gray-400 cursor-not-allowed pointer-events-none" : "relative group"}>
                <Link to="agenda">
                  <BiSolidCalendar className="text-[18px]"></BiSolidCalendar>
                </Link>
          ...
          <div className={isDemo ? "text-gray-400 cursor-not-allowed pointer-events-none" :" flex justify-between gap-1 lg:md:gap-3"}>
            ...
            <div className={isDemo ? " cursor-pointer pointer-events-auto" :""}>
              ...
              <div className="dropdown  lg:md:pr-10 pe-5">
                <label tabIndex={0} className="ease-in-out">
                  <div className="relative group ">
                    <div className="rounded-full transition duration-300 ease-in-out transform hover:scale-105 group-hover:opacity-100 opacity-80">
                      <img
                        className=" border border-orange-700 h-10 w-12 rounded-xl"
                        src={avatarSrc}
                        alt=""
                      />
                    </div>
                ...
                <ul
                  tabIndex={0}
                  className=" -ms-32 menu menu-sm dropdown-content mt-7  p-1 shadow-xl font-semibold  border-black rounded-box w-max"
                >
                  <div className="lg:md:px-1">
                    <li className="border-b-2">
                      <div className="flex gap-1 items-center">
                        ...
                        <h2>
                          <span className="semi-bold">{userEmail}</span>
                          <br />
                        </h2>
                      </div>
                    </li>
                    <li className={isDemo ? "text-gray-400 cursor-not-allowed pointer-events-none" :""}>
                      <Link to="settings/profile"> Profile Settings</Link>
                    </li>

                    {user && (
                      <>
                        <div className={isDemo ? " cursor-pointer pointer-events-auto" : "text-center w-14 mx-auto"}>
                          <li
                            onClick={handleLogOut}
